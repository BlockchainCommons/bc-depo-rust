...
impl DbDepoImpl {
    ...
    fn schema_name(&self) -> &str { &self.schema_name }
}
...
async fn get_settings(
    ...
    match result {
        Some(row) => {
            ...
            let private_keys =
                PrivateKeys::from_ur_string(private_keys_string)?;
            ...
            let continuation_expiry_seconds: u64 =
                row.get("continuation_expiry_seconds").ok_or_else(|| {
                    anyhow!("Continuation expiry seconds not found")
                })?;
...
impl DepoImpl for DbDepoImpl {
    fn max_data_size(&self) -> u32 { self.max_data_size }
    ...
    async fn id_to_receipts(&self, user_id: XID) -> Result<HashSet<Receipt>> {
        ...
        for row in result {
            ...
            let receipt_envelope =
                Envelope::from_ur_string(receipt_string).unwrap();
    ...
    async fn set_user_xid_document(
        ...
        let mut conn = self.pool.get_conn().await?;
        let query = "UPDATE users SET xid_document = :new_xid_document WHERE user_id = :user_id";
        let params = params! {
            "user_id" => user_id.ur_string(),
    ...
    async fn set_user_recovery(
        ...
        let mut conn = self.pool.get_conn().await?;
        let query =
            "UPDATE users SET recovery = :recovery WHERE user_id = :user_id";
        let params = params! {
            "recovery" => recovery,
            "user_id" => user.user_id().ur_string(),
        };
...
fn row_to_user(row: Row) -> User {
    ...
    let xid_document =
        XIDDocument::from_ur_string(xid_document_string).unwrap();
...
pub fn server_pool() -> Pool { Pool::new(server_url().as_str()) }
