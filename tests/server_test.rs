...
use bc_components::{ARID, PrivateKeys, keypair};
use bc_envelope::prelude::*;
use bc_xid::{Key, XIDDocument};
use depo::{Depo, create_db_if_needed, reset_db, setup_log, start_server};
use depo_api::{
    DeleteAccount, DeleteShares, FinishRecovery, GetRecovery,
    GetRecoveryResult, GetShares, GetSharesResult, StartRecovery, StoreShare,
    StoreShareResult, UpdateRecovery, UpdateXIDDocument,
};
use gstp::prelude::*;
use log::{info, warn};
...
impl Context<'_> {
    ...
    fn with_xid_document<'a>(
        &'a self,
        xid_document: &'a XIDDocument,
    ) -> Context<'a> {
...
async fn test_db_depo() {
    ...
    if let Err(e) = create_db_if_needed(schema_name).await {
        warn!(
            "{}",
            Yellow.paint(format!(
                "Skipping `{}` because can't connect to the database.",
                schema_name
            ))
        );
    ...
    let depo_public_key = match get_public_key(&depo).await {
        ...
        Err(e) => {
            warn!(
                "{}",
                Yellow.paint(format!(
                    "Skipping `{}` because can't connect to the depo server.",
                    "test_server_separate"
                ))
            );
            warn!("{}", Yellow.paint(format!("{}", e)));
            return;
...
impl ClientRequestHandler {
    fn new(port: u16) -> Self { Self { client: Client::new(), port } }
}
...
async fn server_call(
    ...
    let id = ARID::new();
    let sealed_request =
        SealedRequest::new_with_body(body.to_expression(), id, sender)
            .with_optional_peer_continuation(peer_continuation);
    let encrypted_request = sealed_request
        .to_envelope(
            None,
            Some(client_private_keys),
            Some(context.depo_xid_document),
        )
        .unwrap();

    let raw_response = context
        .depo
...
async fn server_call_ok(
    ...
    let sealed_response = server_call(
        body,
        client_private_keys,
        sender,
        peer_continuation,
...
async fn server_call_error_contains(
    ...
    if !error_message.contains(expected_error) {
        panic!(
            "🔥 error_message: '{error_message}', does not contain expected: '{expected_error}'"
        );
    }
    sealed_response
}
...
fn hex_bytes(hex: &str) -> ByteString { hex::decode(hex).unwrap().into() }
...
pub async fn test_depo_scenario(context: &Context<'_>) {
    ...
    let (result, _): (StoreShareResult, _) = server_call_ok_into_result(
        &body,
        &alice_private_keys_1,
        &alice_xid_document_1,
        None,
        context,
    )
    .await;
    ...
    let (result, _): (GetSharesResult, _) = server_call_ok_into_result(
        &body,
        &alice_private_keys_1,
        &alice_xid_document_1,
        None,
    ...
    .await;
    assert_eq!(
        result.data_for_receipt(alice_receipt_1),
        Some(&alice_data_1)
    );
    ...
    .await;
    assert_eq!(
        result.data_for_receipt(alice_receipt_2),
        Some(&alice_data_2)
    );
    ...
    let body = GetShares::new_all_shares();
    server_call_error_contains(
        &body,
        &nonexistent_private_keys,
        &nonexistent_xid_document,
        None,
        context,
        "unknown user XID",
    )
    ...
    let body = GetShares::new_all_shares();
    server_call_early_error_contains(
        &body,
        &alice_private_keys_1,
        &alice_xid_document_1,
        None,
        &context.with_xid_document(&nonexistent_xid_document),
        "unknown recipient",
    )
    .await;
    ...
    let (result, _): (GetRecoveryResult, _) = server_call_ok_into_result(
        &body,
        &bob_private_keys_1,
        &bob_xid_document_1,
        None,
    ...
    server_call_error_contains(
        ...
        None,
        context,
        "recovery method already exists",
    )
    .await;
    ...
    alice_xid_document_2
        .add_key(Key::new_allow_all(&alice_public_keys_2))
        .unwrap();
    ...
    server_call_error_contains(
        ...
        None,
        context,
        "invalid sender verification key",
    )
    .await;
    ...
    bob_xid_document_2
        .add_key(Key::new_allow_all(&bob_public_keys_2))
        .unwrap();
    ...
    let sealed_response = server_call_ok(
        &body,
        &bob_private_keys_2,
        &bob_xid_document_2,
        None,
    ...
    alert!(
        "Bob attempts to use the recovery continuation to finish setting his new public key, but the request is incorrectly signed by his old key"
    );
    ...
    section!(
        "Bob uses the recovery continuation to finish setting his new public key, properly signed by his new key"
    );
    let body = FinishRecovery::new();
    server_call_ok(
        &body,
